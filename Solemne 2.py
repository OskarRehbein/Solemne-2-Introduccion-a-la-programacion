...
def mostrar_matriz():
    for i in range(len(turnos_colaboradores)):
        for j in range(len(turnos_colaboradores[0])):
            print(turnos_colaboradores[i][j], end=" ")
        print()

def informacion_fin_de_mes():
    for i in range(numero_colaboradores):
        print(f"El colaborador {i+1} trabajo los dias: ", end=" ")
...
for dia in range(dias_del_mes):
    ...
    while True:
        ...
        if not x in lista_de_turno:
            lista_de_turno.append(x) # type: ignore
            break
    print(lista_de_turno)
    ...
    if len(lista_de_turno) == numero_colaboradores:
        ...
        #Intento limpieza de turnos
        """print(lista_de_turno)
        backup_lista_de_turno.append(lista_de_turno[len(lista_de_turno)-1])
        print(backup_lista_de_turno)
        lista_de_turno.clear()
        lista_de_turno.append(backup_lista_de_turno[len(backup_lista_de_turno)-1])"""
